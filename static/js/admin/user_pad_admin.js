...
function group() {
    ...
    function handlers() {
        ...
        $('#addPadButton').unbind('click').click(function () {
            var pad = {};
...
function groups() {
    ...
    var showGroups = function (groups, sortFunc) {
        ...
        for (var i = 0; i < groups.length; i++) {
            var row = widget.find('.template tr').clone();
            row.find(".ID").html('<a class="groupID">' + groups[i].id + '</a> (InternalID: <span class="groupID-'+groups[i].id+'">loading</span>)');
        ...
        for (var i = 0; i < groups.length; i++) {
            socket.emit("normal-group-to-etherpad-group", groups[i].id, function (success, groupid, internalid) {
               if (success) {
                  var resultList = $('.group-results-div').find('.group-results');
                  resultList.find(".groupID-"+groupid).html(internalid);
               }
            });
    ...
    function handlers() {
        ...
        $('#addGroupButton').unbind('click').click(function () {
    ...
    var showUsers = function (users, sortFunc) {
        ...
        for (var i = 0; i < users.length; i++) {
            ...
            row.find(".newPWButton").bind('click', function (e) {
                var row = $(e.target).closest("tr");
                var id = row.find('.userID').html();
                row.find(".success").html('<img src= "../../static/plugins/ep_user_pads/static/html/wait.gif" width ="12" height = "12" alt="Wait">');
                var val = {};
                val.id = id;
                val.row = row;
                socket.emit("reset-pw-user", val, function (retval) {
                    ...
                    if (retval.success) {
                        row.parent().find('.success').html('<img src= "../../static/plugins/ep_user_pads/static/html/success.jpg" width ="12" height = "12" alt="Success">');
                    } else {
                        row.parent().find('.success').html('<img src= "../../static/plugins/ep_user_pads/static/html/fail.jpg" width ="12" height = "12" alt="Fail">');
                    }
                });
            });
            if (users[i].active) {
